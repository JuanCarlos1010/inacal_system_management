...
public class FormRepository implements BaseRepository<Form, String> {
    ...
    public Optional<Form> findById(String id) {
        try {
            Form result = entityManager.createQuery("FROM Form WHERE id = :id AND deletedAt IS NOT NULL", Form.class)
                    .setParameter("id", id)
                    .getSingleResult();
    ...
    public PageResponse<Form> findAll(Pagination pagination) {
        List<Form> result = entityManager.createQuery("FROM Form WHERE deletedAt IS NULL", Form.class)
                .setFirstResult(pagination.offset())
                .setMaxResults(pagination.getSize())
                .getResultList();
        long count = entityManager.createQuery("FROM Form WHERE deletedAt IS NULL", Long.class)
                .getSingleResult();
